...
class xajax
{
	...
	{	
		...
		if ($bFoundFunction)
		{
			...
			if (isset($aArgs[$i]) && is_string($aArgs[$i])) {
			    if (!function_exists('get_magic_quotes_gpc') || !get_magic_quotes_gpc()) {
			        // Verificamos si las barras invertidas ya están presentes en la variable.
			        // Si no, entonces podemos aplicar stripslashes().
			        if (strpos($aArgs[$i], '\\') !== false) {
	...
	function _parseObjXml($rootTag)
	{
		...
		if ($rootTag == "xjxquery")
		{
			...
			foreach ($aArray as $sKey => $sValue) {
			    if (is_string($sValue)) {
			        if (!function_exists('get_magic_quotes_gpc') || !get_magic_quotes_gpc()) {
			            // Verificamos si las barras invertidas ya están presentes en la cadena.
			            // Si es así, aplicamos stripslashes().
