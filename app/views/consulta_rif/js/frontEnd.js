...
if (createTicketFalla1Btn) { // Verifica si el botón existe
    createTicketFalla1Btn.addEventListener('click', () => {
        if (globalSerial && globalRif) { // Asegura que tenemos los datos
            if(globalEstatusPos !== 'Equipo Desafiliado' && globalEstatusPos !== 'Equipo Inactivo'){
                if (InputRifModal1) InputRifModal1.value = globalRif;
                if (serialSelectModal1) {
                    serialSelectModal1.innerHTML = `<option value="${globalSerial}">${globalSerial}</option>`;
                ...
                const miModalInstance = new bootstrap.Modal(document.getElementById('miModal1'));
                miModalInstance.hide();
                preventDefault();
            }
                // Abre el modal miModal1
                const miModal1Instance = new bootstrap.Modal(document.getElementById('miModal1'));
                miModal1Instance.show();
        } else {
            alert('Por favor, selecciona un POS y asegúrate de tener RIF y Serial para crear el ticket.');
        }
    });
}

// Event Listener para el botón "Crear Ticket Falla 2"
if (createTicketFalla2Btn) { // Verifica si el botón existe
    createTicketFalla2Btn.addEventListener('click', () => {
        if (globalSerial && globalRif) { // Asegura que tenemos los datos
            if(globalEstatusPos !== 'Equipo Desafiliado' && globalEstatusPos !== 'Equipo Inactivo'){
                if (InputRifModal2) InputRifModal2.value = globalRif;
                if (serialSelectModal2) {
                    serialSelectModal2.innerHTML = `<option value="${globalSerial}">${globalSerial}</option>`;
                    // Puedes seleccionar la opción si lo deseas:
                ...
                const miModalInstance = new bootstrap.Modal(document.getElementById('miModal'));
                miModalInstance.hide();
                preventDefault();
            }
  ...
  $(window)
    ...
    .on("click", function (event) {
      if (event.target == modal[0]) {
        ...
        modal1.css("display", "none");
        nivelFallaModal.css("display", "none");
      } else if (event.target == nivelFallaModal[0]) {
...
function SendDataFailure1() {
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
                         ...
                         willClose: () => {
                            ...
                            const beautifulHtmlContent = `
                                <div style="text-align: left; padding: 15px;">
                                    ...
                                    <p style="margin-bottom: 8px;">
                                        <strong>📊 Nivel de Falla:</strong> ${ticketData.nivelFalla}
                                    </p>
                                    <p style="margin-bottom: 8px;">
            ...
            } else if (xhr.status === 429) { // Manejo específico para el código de estado 429 (Too Many Requests)
            try {
                const response = JSON.parse(xhr.responseText);
                Swal.fire({
                    icon: 'warning',
    ...
    const datos = `action=SaveDataFalla&serial=${encodeURIComponent(serial)}&falla=${encodeURIComponent(falla)}&nivelFalla=${encodeURIComponent(nivelFalla)}&id_user=${encodeURIComponent(id_user)}&rif=${encodeURIComponent(rif)}&falla_text=${encodeURIComponent(fallaText)}`;
    xhr.send(datos);
}
