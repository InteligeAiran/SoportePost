...
document.addEventListener("DOMContentLoaded", function () {
    // Precargar logo para PDF (si no está ya cargado)
    try {
        if (!window.PDF_LOGO_DATAURL) {
            ...
            const base = (typeof APP_PATH === 'string' ? APP_PATH : '/');
            img.src = base + 'app/public/img/Nota_Entrega/INTELIGENSA.PNG';
        }
    } catch (_) {}
    // Referencias a los elementos
    ...
    const buscarPorEstatusSelect = document.getElementById("SelectStatus");
    const buscarPorEstatus = document.getElementById("buscarStatus");
    const buscarPorBancoSelect = document.getElementById("SelectBancos");
    const buscarPorBancos = document.getElementById("buscarBancos");
    const ticketInput = document.getElementById("ticketInput");
    const buscarTicketBtn = document.getElementById("buscarTicket");
    const resultsCard = document.querySelector(".card");

    // Función para ocultar todos los campos de búsqueda y limpiar mensajes de error
    function hideAllSearchInputs() {
        // Verificar y ocultar elementos relacionados con RIF
        if (rifTipoSelect) rifTipoSelect.style.display = "none";
        if (rifInput) {
            rifInput.style.display = "none";
            rifInput.value = "";
        }
        if (buscarRifBtn) buscarRifBtn.style.display = "none";
        
        // Verificar y ocultar elementos relacionados con Serial
        if (serialInput) {
            serialInput.style.display = "none";
            serialInput.value = "";
        }
        if (buscarSerialBtn) buscarSerialBtn.style.display = "none";

        // Verificar y ocultar elementos relacionados con Razón
        if (razonInput) {
            razonInput.style.display = "none";
            razonInput.value = "";
        }
        if (buscarRazonBtn) buscarRazonBtn.style.display = "none";

        // Verificar y ocultar elementos relacionados con fechas
        if (inputsDateDiv) inputsDateDiv.style.display = "none";
        if (dateIniInput) {
            dateIniInput.style.display = "none";
            dateIniInput.value = "";
        }
        ...
        if (buscarRangoBtn) buscarRangoBtn.style.display = "none";
        if (errorDateIni) errorDateIni.style.display = "none";
        if (errorDateEnd) errorDateEnd.style.display = "none";

        // Verificar y ocultar elementos relacionados con regiones
        if (selectRegions) {
            selectRegions.style.display = "none";
            selectRegions.value = "";
        }
        if (buscarRegionsBtn) buscarRegionsBtn.style.display = "none";

        // Verificar y ocultar elementos relacionados con estatus
        if (buscarPorEstatusSelect) buscarPorEstatusSelect.style.display = "none";
        if (buscarPorEstatus) buscarPorEstatus.style.display = "none";


        if (buscarPorBancoSelect) buscarPorBancoSelect.style.display = "none";
        if (buscarPorBancos) buscarPorBancos.style.display = "none";
        ...
        if (buscarTicketBtn) buscarTicketBtn.style.display = "none";
    ...
    // Event Listener principal para el select de regiones
    if (regionesSelect) {
        regionesSelect.addEventListener("change", function () {
            ...
            // Mostrar los campos correspondientes según la opción seleccionada
            switch (selectedValue) {
                case "1": // Rango de Fecha
                    if (inputsDateDiv) inputsDateDiv.style.display = "flex";
                    if (dateIniInput) dateIniInput.style.display = "block";
                    if (dateEndInput) dateEndInput.style.display = "block";
                    if (buscarRangoBtn) buscarRangoBtn.style.display = "block";
                    if (resultsCard) {
                        resultsCard.style.display = "block";
                        resultsCard.style.marginTop = "-3%";
                    }
                ...
                case "2": // Serial
                    if (serialInput) serialInput.style.display = "block";
                    if (buscarSerialBtn) buscarSerialBtn.style.display = "block";
                    if (resultsCard) {
                        resultsCard.style.display = "block";
                        resultsCard.style.marginTop = "0%";
                    }
                ...
                case "3": // Rif
                    if (rifTipoSelect) rifTipoSelect.style.display = "block";
                    if (rifInput) rifInput.style.display = "block";
                    if (buscarRifBtn) buscarRifBtn.style.display = "block";
                    if (resultsCard) {
                        resultsCard.style.display = "block";
                        resultsCard.style.marginTop = "5%";
                    }
                    
                    // Agregar validación para el input de RIF solo si jQuery está disponible
                    if (typeof $ !== 'undefined' && rifInput) {
                        $("#rifInput").keyup(function () {
                            let string = $("#rifInput").val();
                            $("#rifInput").val(string.replace(/ /g, ""));
                        });
                ...
                case "4": // Region
                    if (selectRegions) selectRegions.style.display = "block";
                    if (buscarRegionsBtn) buscarRegionsBtn.style.display = "block";
                    if (resultsCard) {
                        resultsCard.style.display = "block";
                        resultsCard.style.marginTop = "0%";
                    }
                ...
                case "5": // Estatus
                    if (buscarPorEstatusSelect) buscarPorEstatusSelect.style.display = "block";
                    if (buscarPorEstatus) buscarPorEstatus.style.display = "block";
                    if (resultsCard) {
                        resultsCard.style.display = "block";
                        resultsCard.style.marginTop = "0%";
                    }
                ...
                case "6": // Bancos
                    if (buscarPorBancoSelect) buscarPorBancoSelect.style.display = "block";
                    if (buscarPorBancos) buscarPorBancos.style.display = "block";
                    if (resultsCard) {
                        resultsCard.style.display = "block";
                        resultsCard.style.marginTop = "0%";
                    }
                ...
                case "7": // Nro de Ticket
                    if (ticketInput) ticketInput.style.display = "block";
                    if (buscarTicketBtn) buscarTicketBtn.style.display = "block";
                    if (resultsCard) {
                        resultsCard.style.display = "block";
                        resultsCard.style.marginTop = "0%";
                    }
                ...
                default: // Opción por defecto (Seleccione Región)
                    // No mostrar nada
                    break;
            }
        });
...
function getBancoClienteUsuarios() {
  const xhr = new XMLHttpRequest();
  xhr.open("POST", `${ENDPOINT_BASE}${APP_PATH}api/consulta/getRegionTicket`);
...
function SendTicket() {
  ...
  if (!nroTicket) {
    Swal.fire({
      icon: "warning",
      title: "Atención",
      text: "Por favor, ingrese un número de ticket.",
      color: "black",
  ...
  // Validation: Check if exactly 10 digits
  if (nroTicket.length !== 10) {
    Swal.fire({
      icon: "warning",
      title: "Atención",
      text: "El número de ticket debe tener exactamente 10 dígitos.",
      color: "black",
      confirmButtonText: "Aceptar",
      confirmButtonColor: "#003594",
    });
  ...
  const xhr = new XMLHttpRequest();
  xhr.open("POST", `${ENDPOINT_BASE}${APP_PATH}api/consulta/globalSearchTicket`);
  xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  ...
  xhr.onload = function () {
    ...
    if (xhr.status >= 200 && xhr.status < 300) {
      try {
        ...
        if (response.success && response.ticket && response.ticket.length > 0) {
          ...
          // Table Generation Logic (Copied and adapted from SendRif)
          ...
          // Determine visible keys based on data existence
          ...
          function showExportLoading() {
            ...
            if (!loadingOverlay) {
              ...
              spinner.style.cssText = `
                width: 80px; height: 80px; border: 6px solid #f3f3f3;
                border-top: 6px solid #003594; border-radius: 50%;
                animation: spin 1s linear infinite; margin-bottom: 20px;
              `;
              const message = document.createElement('h4');
              message.textContent = 'Generando documento...';
              message.style.cssText = 'color: #003594; margin-bottom: 10px; font-weight: 600;';
              const subMessage = document.createElement('p');
              subMessage.textContent = 'Por favor espere, esto puede tardar unos momentos';
              subMessage.style.cssText = 'color: #666; font-size: 14px;';
            ...
            buttons: [
                {
                    ...
                    filename: () => {
                        ...
                        return `REPORTE TICKET - ${nroTicket}_${fecha}`;
                    },
                    title: () => {
                        return `Reporte del Ticket ${document.getElementById("ticketInput").value}`;
                    },
                    ...
                    action: function(e, dt, button, config) {
                        ...
                        const delay = Math.min(Math.max(rowCount * 10, 2000), 10000);
                        setTimeout(function() {
                            hideExportLoading();
                        }, delay);
                    },
    ...
    filename: () => {
        const nroTicket = document.getElementById("ticketInput").value;
        return `REPORTE TICKET - ${nroTicket} - ${new Date().toISOString().split('T')[0]}`;
    },
    title: () => {
        return `Reporte Ticket ${document.getElementById("ticketInput").value}`;
    },
    ...
    action: function(e, dt, button, config) {
        ...
        const rowCount = dt.rows({search: 'applied'}).count();
        const delay = Math.min(Math.max(rowCount * 15, 3000), 15000);
    ...
    customize: function(doc) {
        ...
        customizeTicketPDF(doc, "REPORTE DE TICKET INDIVIDUAL", `Ticket Nro: ${nroTicket}`);
          ...
          $(newTable).resizableColumns();
             ...
             if (welcomeMessage) {
                welcomeMessage.style.visibility = "visible"; // typo fixed: visble -> visible
                welcomeMessage.style.opacity = "1";
            }
             mainTableCard.innerHTML = `
            <div class="table-responsive">
                 <table id="rifCountTable" class="background-users-table" style="width:100%">
                    <thead></thead>
                    <tbody>
                        <tr> <td colspan="15" class="text-center">No se encontraron datos para el ticket ${nroTicket}</td> </tr>
                    </tbody>
                </table>
            </div>`;
        }
...
function SendRegions() {
  ...
  xhr.onload = function () {
            ...
            buttons: [
    {
        ...
        title: () => `Reporte del Ticket ${document.getElementById("ticketInput").value}`,
        className: 'btn-excel-modern',
        attr: { id: 'btn-excel-modern-id', title: 'Exportar a Excel' },
        action: function(e, dt, button, config) {
            showExportLoading();
            $.fn.dataTable.ext.buttons.excelHtml5.action.call(this, e, dt, button, config);
            const rowCount = dt.rows({search: 'applied'}).count();
    ...
    },
    {
        ...
        title: () => `Reporte Ticket ${document.getElementById("ticketInput").value}`,
        className: 'btn-pdf-modern',
...
function SendRif() {
  ...
  xhr.onload = function () {
    ...
    const rifCompleto = numeroRif ? `${tipoRif}${numeroRif}` : 'SIN_RIF';
    const fecha = new Date().toISOString().split('T')[0];
    return `REPORTE DE TICKET - RIF ${rifCompleto} - ${fecha}`;
  },
    ...
    action: function(e, dt, button, config) {
        ...
        $.fn.dataTable.ext.buttons.pdfHtml5.action.call(this, e, dt, button, config);
        const rowCount = dt.rows({search: 'applied'}).count();
        const delay = Math.min(Math.max(rowCount * 18, 4000), 18000);
        setTimeout(hideExportLoading, delay);
    },
    exportOptions: {
        columns: ':visible',
        format: { 
            body: data => (typeof data === 'string' ? data.replace(/<[^>]*>/g, '').trim() : data) 
        }
    },
    customize: function(doc) {
        const tipoRif = document.getElementById('rifTipo')?.value?.toUpperCase() || 'V';
...
function SendSerial() {
  ...
  xhr.onload = function () {
    ...
    filename: () => {
        const serial = document.getElementById('serialInput')?.value?.trim().toUpperCase() || 'SIN_SERIAL';
        const fecha = new Date().toISOString().split('T')[0];
        return `REPORTE DE TICKET - SERIAL ${serial} - ${fecha}`;
    },
    title: 'Rporte por Serial',
    className: 'btn-pdf-modern',
    attr: {
        id: 'btn-pdf-modern-id',
        title: 'Exportar a PDF'
...
},
{
  ...
  filename: `REPORTE TICKETS - ESTATUS ${EstatusName.toUpperCase()} - ${new Date().toISOString().split('T')[0]}`,    className: 'btn-pdf-modern',
    attr: {
        id: 'btn-pdf-modern-id',
        title: 'Exportar a PDF'
    },
    action: function(e, dt, button, config) {
        ...
        oldConfig.exportOptions = oldConfig.exportOptions || {};
        $.fn.dataTable.ext.buttons.pdfHtml5.action.call(this, e, dt, button, oldConfig);
    ...
    if (isPDF) {
        ...
        return {
            stack: [
                {
                    columns: [
                        { image: 'logo_inteligensa', width: 90 },
                        { text: 'RIF: J-00291615-0', alignment: 'right', bold: true, color: '#1f4e8c', fontSize: 10 }
                    ],
                    margin: [40, 20, 40, 10]
                },
                { canvas: [{ type: 'line', x1: 40, y1: 5, x2: 555, y2: 5, lineWidth: 1, lineColor: '#d9d9d9' }] },
                {
                ...
                currentPage === 1 ? {
                    text: 'REPORTES DE TICKETS POR RANGO DE FECHA',
                    alignment: 'center',
                    fontSize: 16,
                    bold: true,
                    color: '#1f4e8c',
        ...
        doc.content.push(
            ...
            {
                canvas: [
                    { type: 'rect', x: 0, y: 0, w: 515, h: 70, color: '#f8f9fa', r: 12 }
                ],
            ...
            },
            {
                ...
                bold: true,
                color: '#d32f2f',
                absolutePosition: { x: 60, y: 200 }
            },
            {
                text: 'Los campos que aparecen como "NULL" indican que no existe información disponible para ese dato.',
                fontSize: 10.5,
                italics: true,
                color: '#424242',
                alignment: 'left',
                lineHeight: 1.4,
                absolutePosition: { x: 60, y: 225 },
                margin: [0, 0, 60, 0]
            },
            // Línea debajo
            {
            ...
            if (tableIndex !== -1) {
                ...
                if (body && body.length > 1) {
                    ...
                    dataRows.forEach((row, idx) => {
                        ...
                        if (label.includes('ID Ticket') || label === 'ID_TICKET') idTicketValue = value;
                        if (label.includes('Razón Social') || label.includes('RAZON SOCIAL')) razonSocialValue = value;
                        if (label === 'Rif' || label.includes('RIF')) rifValue = value;

                        kvRows.push([
                            { text: label || '-', style: 'labelCell', margin: [4, 3, 6, 3] },
                            { text: value || '-', style: 'valueCell', margin: [0, 3, 4, 3] }
                        ]);
                    }

                    const sectionTitle = idTicketValue ? `Ticket ${idTicketValue}` : `Registro ${idx + 1}`;
                    const subtitle = [razonSocialValue, rifValue ? `RIF: ${rifValue}` : ''].filter(Boolean).join(' • ');

                    verticalSections.push({
                        stack: [
                            { text: sectionTitle, style: 'sectionTitle' },
                            subtitle ? { text: subtitle, style: 'sectionSubtitle' } : {},
                            {
                                table: { widths: [160, '*'], body: kvRows },
                                layout: {
                                    fillColor: i => i % 2 === 0 ? '#f5f9ff' : null,
                                    hLineWidth: () => 0.5,
                                    vLineWidth: () => 0,
                                    hLineColor: () => '#c8d6ef',
                                    paddingLeft: () => 6,
                        ...
                        margin: [0, 15, 0, 20],
                        pageBreak: (idx + 1) % 4 === 0 ? 'after' : undefined
                    });
                });
                ...
                },
{
    ...
    // NOMBRE DEL ARCHIVO CON RANGO DE FECHAS (TU OPCIÓN FAVORITA)
    filename: () => {
        const ini = document.getElementById('date-ini').value || 'FechaIni';
        const fin = document.getElementById('date-end').value || 'FechaFin';
        return `REPORTE TICKET - RangoFecha ${ini} al ${fin}`;
    ...
    action: function(e, dt, button, config) {
        showExportLoading();
        $.fn.dataTable.ext.buttons.pdfHtml5.action.call(this, e, dt, button, config);
        const delay = Math.min(Math.max(dt.rows({search: 'applied'}).count() * 12, 2500), 12000);
        setTimeout(hideExportLoading, delay);
    },
    exportOptions: {
        columns: ':visible',
...
function getBancoTicket() {
  ...
  xhr.onload = function () {
    if (xhr.status === 200) {
      try {
        const response = JSON.parse(xhr.responseText);
        if (response.success) {
          const select = document.getElementById("SelectBancos");

          select.innerHTML = "";

          select.innerHTML = '<option value="">Seleccione</option>'; // Limpiar y agregar la opción por defecto
          if (
            Array.isArray(response.ticket) &&
            response.ticket.length > 0
          ) {
            response.ticket.forEach((ticket) => {
              const option = document.createElement("option");
              option.value = ticket.codbanco;
              option.textContent = ticket.descbanco;
              select.appendChild(option);
            });
          } else {
            // Si no hay fallas, puedes mostrar un mensaje en el select
            const option = document.createElement("option");
            option.value = "";
            option.textContent = "No hay información ";
            select.appendChild(option);
          }
        } else {
          document.getElementById("rifMensaje").innerHTML +=
  ...
  const datos = `action=GetBancoTicket`; // Cambia la acción para que coincida con el backend
  xhr.send(datos);
}
...
function SendBancos() {
  ...
  const selectBanco = document.getElementById("SelectBancos").value;
  ...
  const xhr = new XMLHttpRequest();
  xhr.open("POST", `${ENDPOINT_BASE}${APP_PATH}api/reportes/SearchBancoData`);
  xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  ...
  xhr.onload = function () {
    ...
    // DEFINIMOS bancoNombre AQUÍ PARA QUE ESTÉ DISPONIBLE EN TODO EL ÁMBITO
  ...
  filename: `REPORTE TICKET - BANCO ${bancoNombre.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}`,
  className: 'btn-pdf-modern',
  attr: { id: 'btn-pdf-modern-id', title: 'Exportar a PDF' },
  action: function(e, dt, button, config) {
    showExportLoading();
