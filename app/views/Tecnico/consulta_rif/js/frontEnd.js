...
function SendRif() {
    const xhr = new XMLHttpRequest();
    xhr.open('POST', 'http://localhost/SoportePost/api/SearchRif');
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            ...
            try {
                ...
                if (response.success) {
                    ...
                    rifData.forEach(item => { // Usa un nombre diferente para el elemento individual
                        ...
                        garantiaLabel.style.width = '135px';
                        let garantiaTexto = '';
                        let garantiaClase = '';
...
function fetchSerialData(serial) {
    const xhr = new XMLHttpRequest();
    xhr.open('POST', 'http://localhost/SoportePost/api/SearchSerial');
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        ...
        if (xhr.status >= 200 && xhr.status < 300) {
            try {
                ...
                if (response.success && response.serial && response.serial.length > 0) {
                    ...
                    downloadImageModal(serial); // <--- AQUÍ ESTÁ LA LLAMADA A downloadImageModal()
                    ...
                    $(document).ready(function() {
                        ...
                        $('#serialCountTable').DataTable({
                            responsive: true,
                            "pagingType": "simple_numbers",
                            autoWidth: false, // Añade esta línea
                            "language": {
                                "emptyTable": "No hay datos disponibles en la tabla",
                                "zeroRecords": "No se encontraron resultados para la búsqueda",
                                "info": "Mostrando pagina _PAGE_ de _PAGES_ ( _TOTAL_ dato(s) )",
            ...
            try {
                ...
                if (response.success) {
                    ...
                    if (Array.isArray(response.coordinadores) && response.coordinadores.length > 0) {
                        response.coordinadores.forEach(failure => {
                            const option = document.createElement('option');
                            option.value = failure.id_user;
                            option.textContent = failure.full_name;
                            select.appendChild(option);
                        });
                    } else {
                        // Si no hay fallas, puedes mostrar un mensaje en el select
...
function downloadImageModal(serial) {
    const xhr = new XMLHttpRequest();
    xhr.open('POST', 'http://localhost/SoportePost/api/GetPhoto');
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
