  ...
  xhr.onload = function () {
    ...
    render: function (data, type, row) {
        ...
        if (currentStatusDomiciliacion == 4) {
            ...
            // (Solo Lectura: se omite el botón de adjuntar, solo queda visualizar)
            return `
                <div class="d-flex gap-1">
                    <button type="button" class="btn btn-info btn-sm visualizar-documento-btn"
                        data-id="${idTicket}"
          ...
          if (TicketData && TicketData.length > 0) {
            ...
            $("#tabla-ticket tbody")
              ...
              $("#tabla-ticket tbody").on("click", ".visualizar-documento-btn", function () {
                ...
                // Solo Lectura: sin importar el estatus, nunca se muestran los botones
                // de Aprobar/Rechazar dentro del visor (la lógica de arriba solo decide
                // visibilidad por estatus, no por permisos).
                if (typeof isReadOnlyUser !== 'undefined' && isReadOnlyUser) {
                  if (BotonAprobar) BotonAprobar.style.display = 'none';
                  if (BotonRechazo) BotonRechazo.style.display = 'none';
                }
...
function showViewModal(ticketId, nroTicket, imageUrl, pdfUrl, documentName, currentStatusId = null) {
    ...
    // Solo Lectura: sin importar el estatus, nunca se muestran los botones de
    // Aprobar/Rechazar dentro del visor.
    if (typeof isReadOnlyUser !== 'undefined' && isReadOnlyUser) {
        if (BotonAprobar) BotonAprobar.style.display = 'none';
        if (BotonRechazo) BotonRechazo.style.display = 'none';
    }
