...
function GetRegionUser(id_user) {
    ...
    if (xhr.status === 200) {
      try {
        ...
        if (response.success) {
          const inputRegnew = document.getElementById("InputRegionUser2");
          const inputReg = document.getElementById("InputRegion");
          if (inputRegnew) inputRegnew.value = response.user.name_region;
          if (inputReg) inputReg.value = response.user.name_region;
        }
      } catch (e) {
        console.error(e);
      }
  ...
  const data = `action=GetRegionUsersAssign&id_user=${id_user}`;
  xhr.send(data);
}
