...
class tecnico extends Controller {
    ...
    // Política de Referer removida: destruía la sesión en cualquier navegación
    // directa legítima (F5, bookmark, pestaña nueva). Ver dashboard.php para el
    // detalle; CSRF ya lo cubre Controller::validateCSRFToken().

        Model::exists('consulta_rif');
        Model::exists('login'); // Si Model::exists() carga la clase, esto es bueno.
        ...
        $this->view->js = array('tecnico/js/frontEnd.js');
        $this->view->render('tecnico/index', 1);
    }
