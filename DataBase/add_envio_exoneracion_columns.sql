-- =====================================================================
-- FEATURE: pdf_path_envio y pdf_path_exoneracion en getdataticketfinal()
-- El botón "Visualizar Documentos" de Gestión Rosal (pendiente_entrega)
-- solo mostraba Envío a Destino, Presupuesto e Informe Técnico. Se
-- detectó que tickets con documento de "Envio" (envío inicial, subido
-- al crear el ticket) o "Exoneracion" (módulo de pagos) no mostraban
-- esos documentos porque la función nunca los traía. Se agregan como
-- dos columnas más, con el mismo patrón de LEFT JOIN LATERAL que ya
-- se usa para presupuesto e informe_tecnico.
--
-- IMPORTANTE: como esta función usa RETURNS TABLE y se le agregan
-- columnas nuevas, Postgres NO permite un simple CREATE OR REPLACE
-- (cambia el tipo de retorno) -- hay que hacer DROP FUNCTION primero.
-- =====================================================================
...
CREATE OR REPLACE FUNCTION public.getdataticketfinal(
    ...
    RETURNS TABLE(
        ...
        -- Informe Técnico (equipos Irreparables)
        pdf_path_informe_tecnico character varying,
        -- Envío inicial (subido al crear el ticket) y Exoneración (módulo de pagos)
        pdf_path_envio character varying,
        pdf_path_exoneracion character varying
    )
    LANGUAGE 'plpgsql'
    COST 100
