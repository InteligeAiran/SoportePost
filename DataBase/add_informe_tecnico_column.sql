-- =====================================================================
-- FEATURE: pdf_path_informe_tecnico en getdataticketfinal()
-- Para equipos marcados como "Gestión Comercial (Irreparable)" en el
-- Rosal, el flujo ya no exige el documento de Presupuesto; en su lugar
-- exige un "Informe Técnico" (nuevo document_type = 'informe_tecnico'
-- en archivos_adjuntos). Se agrega la columna pdf_path_informe_tecnico
-- vía un LEFT JOIN LATERAL, igual que se hizo para pdf_path_presupuesto.
--
-- IMPORTANTE: como esta función usa RETURNS TABLE y se le agrega una
-- columna nueva, Postgres NO permite un simple CREATE OR REPLACE
